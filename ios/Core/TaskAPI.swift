...
final class TaskAPI: Sendable {
    ...
    private func call<T: Decodable>(_ path: String, method: String, body: [String: Any]? = nil, as: T.Type) async throws -> T { try await decode(request(path, method: method, data: body.map { try! JSONSerialization.data(withJSONObject: $0) }), as) }
    private func call<T: Decodable, E: Encodable>(_ path: String, method: String, encodable: E, as: T.Type) async throws -> T { try await decode(request(path, method: method, data: try JSONEncoder().encode(encodable)), as) }
